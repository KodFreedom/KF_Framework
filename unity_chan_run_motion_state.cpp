...
void UnityChanRunMotionState::ChangeMotion(Animator& animator)
{
	...
	{
		animator.Change(MY_NEW BlendMotionState(current_motion_name_, MY_NEW UnityChanWalkMotionState(0), current_frame_counter_, 10));
		return;
	}
	...
	{
		animator.Change(MY_NEW BlendMotionState(current_motion_name_, MY_NEW UnityChanJumpMotionState(0), current_frame_counter_, 5));
		return;
	}
}